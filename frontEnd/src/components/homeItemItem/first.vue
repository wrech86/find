...
let data = reactive(props.one)
function showLike(e, index) {
  data[index].checked = !data[index].checked
  if (data[index].checked) {
    e.target.style.color = 'red'
    store.commit('addLikeInfo',data[index].oblong)
  } else {
    e.target.style.color = '#cccccc'
    store.commit('removeLikeInfo',data[index].oblong)
  }
}
...
function toText(item, index) {
  router.push({ name: 'classifyText', query: { key: props.one[index].key, flag: props.one[index].flag } })
}
</script>
